...
//console.log(type)
        ...
        if(atype.startsWith('!render-')) {
          ...
          switch(atype) {
              ...
              case "!end" : 
                  //console.log("!end", x);
                  return getEnd(config.onend)+bodyEnd(version)+(x)+"\n</body></html>";
              case "!--": return "";
              default: 
...
//    var linesPromise = pipe(splitLines,getIncludesPromise(new Map(), readLines, srcDir, inputFile, 0))(bodyText);
    ...
    return lines.then(lines => {
                ...
                //lines.map((l,ix) => (ix < 50 || ix >= lines.length-50)?console.log(':',ix,l):undefined);
                // if(lines.some(v => v instanceof Promise)) {
                //   console.log("found some promise");
                //   lines.forEach((l,ix) => {if(l instanceof Promise) around(lines,ix)});
                // }

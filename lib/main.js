...
<div class="execution progress" 
     style="display: none; background-color: #f5f0f0; position: fixed; margin-left: 25rem; top: 3rem; width:60%; border: 3px solid #393977; padding: 8px ">
<div style="margin: auto; color: red; width: 50%">Executing previous dependent scripts</div> 
...
//console.log(type)
        ...
        if(atype.startsWith('!render-')) {
          ...
          switch(atype) {
              ...
              case "!end" : 
                  //console.log("!end", x);
                  return getEnd(config.onend)+bodyEnd(version)+(x)+progressHTML+"\n</body></html>";
              case "!--": return "";
              default: 
...
function getEnd(strOrList) {
  if(!strOrList) return '';
  if(Array.isArray(strOrList)) return strOrList.join('\n');
  return strOrList;
}
...
//    var linesPromise = pipe(splitLines,getIncludesPromise(new Map(), readLines, srcDir, inputFile, 0))(bodyText);
    var linesPromise = pipe(splitLines,getIncludesPromise(new Map(), readLines, srcDir, inputFile))(bodyText);

...
const {defaultOverride, includeFromUrl, shouldInclude} = require('./IncludeManager')
 ...
 */
function includeFileOrUrl(aPath, type, override) {
  override = override || defaultOverride;
  if(!override.toOverride && override.included(aPath)) return Promise.resolve([]);
  let promise = (isURL(aPath) || type === 'URL')? 
        includeFromUrl(aPath, override) : 
        Promise.resolve(getIncludeFromFile(aPath, override));
  return promise.then(s => s.replace(/\r/g, '').split('\n'));
  // if(isURL(aPath)) return includeFromUrl(aPath);

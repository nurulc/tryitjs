...
//            const res = fn.filter(s => firstLine(readLines(s).filter(valid)).trim().startsWith('!head'));
//            resolve(res);
//         });
//     });
// }
...
function tryFilesPromise(srcDir, ext='try') {
  if(!srcDir) srcDir = '.';
  return new Promise((resolve, reject) => {
      glob(srcDir+'/**/**.'+ext,{ ignore: 'node_modules/**' }).then(files => {
         const valid = s =>  !(s=s.trim(), (!s || s.startsWith('!--')));
 ...
 * @param  {boolean} doNotChange  true - do not overwrite an existing file

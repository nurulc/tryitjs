...
module.exports.sections = function (_lines) {
    // console.log(_lines);
    let lines = _lines.flatMap(getMarkers());
    let lastLine = '';
...
function removePrefixStr() {
    ...
    return ((current) => {
        ...
        for(let i=0; i<pl.length; i++) {
            if(pl[i] !== cl[i]) return cl.slice(i).join('/');
        }
        return current;
    });
}
...
class Stack {
    ...
    push(v) {
        ...
        if(v.startsWith('!')) this.state = v.trim();
